...
// OnboardingCluster of that copy), and the logical cluster name is carried in
// the context so platform-side MCP namespaces are derived per tenant
// (StableMCPNamespaceCtx). Identically named ControlPlanes in different
// workspaces therefore never collide on the platform cluster.
func (o *RunOptions) runMulticluster(ctx context.Context, setupLog logging.Logger) error {
	...
	err = mcbuilder.ControllerManagedBy(mcMgr).
		...
		Complete(mcreconcile.Func(func(ctx context.Context, req mcreconcile.Request) (ctrl.Result, error) {
			...
			rc := *mcpRec
			rc.OnboardingCluster = clusters.NewTestClusterFromClient(clusterName, cl.GetClient())
			ctx = libutils.WithTenantCluster(ctx, clusterName)
