...
func loadWorkspaceProviders(path string) ([]workspaceProvider, error) {
	...
	for i := range providers {
		...
		for ruleIndex, rule := range provider.ClusterRoleRules {
			if len(rule.APIGroups) == 0 || len(rule.Resources) == 0 || len(rule.Verbs) == 0 {
				return nil, fmt.Errorf("service provider %q clusterRoleRules[%d] must set apiGroups, resources, and verbs", provider.Name, ruleIndex)
			}
		}
	}
